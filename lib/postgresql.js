...
PostgreSQL.prototype._buildConditionForRelation = function (parentModel, parentTableName, relationName) {
  ...
  switch (relation.type) {
    ...
    case 'hasMany':
      ...
      return this.escapeName(parentTableName) + '.' + this.columnEscaped(parentModel, relation.keyFrom) + '=' + this.columnEscaped(relation.modelTo.name, relation.keyTo);
    default:
...
PostgreSQL.prototype._buildSelectWithAnchorToTable = function (parentModel, parentTableName, relationName, filter, options) {
  const model = this.getModelDefinition(parentModel).relations[relationName].modelTo.name;
...
PostgreSQL.prototype._buildSelectWithInclude = function (model, filter, options) {
  ...
  while (selectsToProccess.length) {
    ...
    const sql = select.parentModel ?
      this._buildSelectWithAnchorToTable(select.model, select.parentModel, select.filter, options) :
    ...
    const fields = this._fieldsFromFilter(select.model, select.scope);
    ...
    if (select.filter.include) {
      selectsToProccess.push(...select.filter.include(include => ({
  ...
  subSelects.forEach(select => {
    ...
    copySql.sql = '(' + copySql.sql + ')';

    requestSql.merge(copySql, ' CROSS JOIN LATERAL ');
  });
...
PostgreSQL.prototype.all = function find(model, filter, options, cb) {
  ...
  this.execute(stmt.sql, stmt.params, options, function(err, data) {
    ...
    if (filter && filter.include) {
      this._constructResponse(model, objs, filter, options, cb);
    } else {
      cb(null, objs);
    }
  });
