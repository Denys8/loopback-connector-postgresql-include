...
PostgreSQL.prototype._normalizeIncludes = function(include) {
  ...
  if (typeof include === 'string') {
    ...
    for (let i = 0, n = include.length; i < n; i++) {
      const subInclude = this._normalizeIncludes(include[i]);
      newInclude = [...newInclude, ...subInclude];
    }
    return newInclude;
  } else {
    return include;
...
PostgreSQL.prototype._checkIncludes = function(model, include) {
  ...
  if (Array.isArray(include)) {
    const relations = Object.keys(currentModel.settings.relations);
    for (const rel of include) {
      if (relations.indexOf(rel.relation) === -1) {
