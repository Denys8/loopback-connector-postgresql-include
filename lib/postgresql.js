...
PostgreSQL.prototype._buildQueryParts = function (model, filter, options) {
  ...
  while (selectsToProccess.length) {
    ...
    const tableName = select.parentTable ? this._generateHashFromParentTable(select.parentTable) + '_' + select.model : select.model;
        ...
        .then(() => {
          ...
          return Promise.all(filter.include.map(include =>
            self._extractModelsFromDataAsync(
              ...
              include.scope,
              options,
              dataPrefix ? self._generateHashFromParentTable(dataPrefix) + '_' + include.relation : self._generateHashFromParentTable(model) + '_' + include.relation,
