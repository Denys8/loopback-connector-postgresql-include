...
PostgreSQL.prototype._buildConditionForRelation = function (parentModel, parentTableName, relationName) {
  ...
  switch (relation.type) {
    case 'belongsTo':
      return parentTableName + '.' + this.columnEscaped(parentModel, relation.foreignKey) + '=' + this.columnEscaped(relation.model, 'id');
    case 'hasOne':
    case 'hasMany':
      ...
      return parentTableName + '.' + this.columnEscaped(parentModel, 'id') + '=' + this.columnEscaped(relation.model, relation.foreignKey);
    default:
      throw new Error(`Relation: ${relation.type} is not implemented yet`);
  }
};

PostgreSQL.prototype._buildSelectWithAnchorToTable = function (parentModel, parentTableName, relationName, filter, options) {
...
PostgreSQL.prototype._buildFields = function (queryParts) {
  ...
  queryParts.forEach(part => {
    fields.push(...part.fields.map(field => part.isMain ?
...
PostgreSQL.prototype._buildQueryParts = function (model, filter, options) {
  ...
  while (selectsToProccess.length) {
    ...
    const currentModel = select.parentModel ? this.getModelDefinition(select.parentModel).settings.relations[select.model].model : select.model;
    ...
    const order = filter.order || this.idNames(model);
    ...
    if (select.filter.include) {
      selectsToProccess.push(...select.filter.include.map(include => ({
        model: include.relation,
        filter: include.scope || {},
        parentTable: tableName,
        parentModel: select.model,
      })));
    }
  }
...
PostgreSQL.prototype._buildSelectWithInclude = function (model, filter, options) {
  ...
  requestSql.sql = 'WITH ' + mainSelect.tableName + ' AS (' + requestSql.sql + ')' +
    'SELECT ' + this._buildFields(queryParts) + ' FROM ' + mainSelect.tableName;
  subSelects.forEach(select => {
    const copySql = new ParameterizedSQL(select.sql.sql, select.sql.params);

    copySql.sql = '(' + copySql.sql + ') AS ' + select.tableName + ' ON TRUE';

    requestSql.merge(copySql, ' LEFT JOIN LATERAL ');
  });

  const orders = [];
  queryParts.forEach(part => {
    if (part.order && part.order.length) {
      orders.push(...part.order.map(order => {
        const t = order.split(/[\s,]+/);

        return t.length === 1 ? part.tableName + '.' + this.columnEscaped(part.model, t[0]) : part.tableName + this.columnEscaped(part.model, t[0]) + t[1];
      }));
    }
  });
...
PostgreSQL.prototype._constructResponse = function (model, data, filter, options, cb) {
  this._extractModelsFromData(this, model, data, filter, options, null, (err, objs) => {
    cb(err, objs);
  });
};

PostgreSQL.prototype._extractModelsFromData = function (self, model, data, filter, options, dataPrefix, cb) {
  filter = filter || {};
  ...
  const objs = data.map(function (row) {
    return self.fromRowWithPrefix(model, row, dataPrefix);
  });
  ...
  if (Array.isArray(filter.include) && filter.include.length) {
    Promise.all(sortedGroups.map(group =>
      Promise.all(filter.include.map(include =>
        self._extractModelsFromDataAsync(
          self,
          modelDefinition.settings.relation[include.relation].model,
          group.rows,
          include.scope,
          dataPrefix + include.relation,
        )
      ))
        .then(relations => {
          const obj = group.obj;
...
PostgreSQL.prototype.fromRowWithPrefix = function (model, rowData, prefix) {
  ...
  if (prefix && Object.keys(rowData).filter(key => key.startsWith(prefix)).every(key => !rowData(key))) {
    return null;
  }
