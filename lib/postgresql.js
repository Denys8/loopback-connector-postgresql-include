...
PostgreSQL.prototype._incrementParamsInSQL = function (sql, incrementBy) {
  ...
  newSql += sqlCopy.substring(lastIndex)
...
PostgreSQL.prototype._buildSelectWithInclude = function (model, filter, options) {
  ...
  subSelects.forEach((select) => {
    requestSql.sql += requestSql.sql + ' LEFT JOIN LATERAL (' + this._incrementParamsInSQL(select.sql.sql, requestSql.params.length) + ') AS ' + select.tableName + ' ON TRUE';
